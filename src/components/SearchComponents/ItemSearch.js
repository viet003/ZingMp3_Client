...
const ItemSearch = ({ item, setOpen }) => {
    ...
    return (
        <div
            ...
            className={`${curSongId === item?.encodeId ? 'bg-sidebarbg' : 'bg-transparent'} grid grid-cols-[20%,55%,15%] gap-2 p-2 rounded-md hover:bg-white`}>
            <div
                className='relative rounded-md overflow-hidden h-[50px] w-[50px]'>
            ...
            <div className='overflow-hidden flex flex-col justify-center'>
                <p className='truncate text-ellipsis text-[13px] font-semibold text-gray-600 cursor-pointer hover:text-primary'>{item?.title}</p>
                <p className='truncate text-ellipsis text-[11px] text-gray-500 cursor-default'>{item?.artistsNames}</p>
            </div>
